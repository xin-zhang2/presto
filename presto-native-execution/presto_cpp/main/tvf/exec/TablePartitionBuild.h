...
class TablePartitionBuild {
 public :
  ...
  /// Check hasNextPartition() before invoking this function. This function fails if
 ...
 private:
  ...
  bool compareRowsWithKeys(
      const char* lhs,
      const char* rhs,
      const std::vector<std::pair<velox::column_index_t, velox::core::SortOrder>>& keys);
  ...
  /// The RowContainer holds all the input rows in TablePartitionBuild. Columns are
  ...
  std::vector<std::pair<velox::column_index_t, velox::core::SortOrder>> allKeyInfo_;
  ...
  std::vector<velox::vector_size_t, velox::memory::StlAllocator<velox::vector_size_t>>
      partitionStartRows_;
