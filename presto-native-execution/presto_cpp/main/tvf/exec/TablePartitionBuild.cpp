...
vector_size_t TablePartitionBuild::findNextPartitionStartRow(vector_size_t start) {
...
std::shared_ptr<TableFunctionPartition> TablePartitionBuild::nextPartition() {
  VELOX_CHECK(!partitionStartRows_.empty(),
              "No table function partitions available");
