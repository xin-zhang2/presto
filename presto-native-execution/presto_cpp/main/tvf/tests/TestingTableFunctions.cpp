...
#include "presto_cpp/main/tvf/tests/TestingTableFunctions.h"
...
void registerIdentityFunction(const std::string& name) {
  TableArgumentSpecList argSpecs;
  argSpecs.insert(std::make_shared<TableArgumentSpecification>("INPUT", true));
  registerTableFunction(
      name,
      argSpecs,
      std::make_shared<GenericTableReturnType>(),
...
std::shared_ptr<TableFunctionResult> RepeatFunctionDataProcessor::apply(
  ...
  outputTable->resize(numRows * handle_->count_);
  for (int i = 0; i < handle_->count_; i++) {
