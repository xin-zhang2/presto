...
protected:
  std::unordered_map<std::string, std::shared_ptr<Argument>> simpleTableFunctionArgs(const std::string& column) {
    std::unordered_map<std::string, std::shared_ptr<Argument>> args;
    args.emplace("COLUMN", std::make_shared<ScalarArgument>(VARCHAR(), makeConstant(StringView(column), 1, VARCHAR())));
    return args;
  }
};
...
TEST_F(TableFunctionInvocationTest, DISABLED_simple) {
  ...
  .addNode(addTvfNode("simple_table_function", simpleTableFunctionArgs("col")))
  .planNode();
...
TEST_F(TableFunctionInvocationTest, repeat) {
  ...
  std::shared_ptr<ScalarArgument> count = std::make_shared<ScalarArgument>(BIGINT(), makeConstant(static_cast<int64_t>(2), 1, BIGINT()));
...
TEST_F(TableFunctionInvocationTest, repeatPartitionOrder) {
  ...
  std::vector<core::FieldAccessTypedExprPtr> partions = {
    std::make_shared<core::FieldAccessTypedExpr>(BIGINT(), "c0")
  };
  std::vector<core::FieldAccessTypedExprPtr> sortings = {
  ...
  auto plan = PlanBuilder()
  .values({data})
  .addNode(addTvfNode("repeat_table_function", args, partions, sortings, sortOrders))
          .planNode();
