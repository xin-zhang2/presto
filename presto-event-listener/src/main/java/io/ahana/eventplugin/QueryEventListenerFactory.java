        ...
        implements EventListenerFactory
{
    ...
    public static final String PRESTO_QUERY_EVENT_LISTENER = "watsonx.data-listener";
    ...
    public static final String EXPORTER_ENDPOINT = "exporter-endpoint";
    public static final String MAX_EXPORTER_BATCH_SIZE = "max-exporter-batch-size";
    public static final String MAX_QUEUE_SIZE = "max-queue-size";
    public static final String SCHEDULE_DELAY = "schedule-delay";
    public static final String EXPORTER_TIMEOUT = "exporter-timeout";
    public static final String TRACE_SAMPLING_RATIO = "trace-sampling-ratio";
    public static final String KAFKA_BROKERS = "kafka-brokers";
    public static final String GUARDIUM_EVENT_KAFKA_TOPIC = "guardium-event-kafka-topic";
    public static final String CES_TRUST_STORE_PATH = "ces-truststore-path";
    public static final String CES_TRUST_STORE_PASSWORD = "ces-truststore-password";
    public static final String CES_TRUST_STORE_TYPE = "ces-truststore-type";
    private static final String GUARDIUM_ENABLED = "guardium-enabled";
    public static final String FINOPS_ENABLED = "finops-enabled";
    public static final String FINOPS_EVENT_KAFKA_TOPIC = "finops-event-kafka-topic";
    ...
    public EventListener create(Map<String, String> config)
    {
        ...
        boolean tracingEnabled = getBooleanConfig(config, TRACING_ENABLED, false);
        if (tracingEnabled) {
            requireNonNull(config.get(EXPORTER_ENDPOINT), "exporter endpoint cannot be null");
            requireNonNull(config.get(MAX_QUEUE_SIZE), "max queue size cannot be null");
            requireNonNull(config.get(MAX_EXPORTER_BATCH_SIZE), "max exporter batch size cannot be null");
            requireNonNull(config.get(EXPORTER_TIMEOUT), "exporter timeout cannot be null");
            requireNonNull(config.get(SCHEDULE_DELAY), "schedule delay cannot be null");
            requireNonNull(config.get(TRACE_SAMPLING_RATIO), "sampling ratio cannot be null");
        }

        boolean guardiumEnabled = getBooleanConfig(config, GUARDIUM_ENABLED, false);
        boolean finOpsEnabled = getBooleanConfig(config, FINOPS_ENABLED, false);
        if (guardiumEnabled) {
            requireNonNull(config.get(KAFKA_BROKERS), "Kafka servers cannot be null");
            requireNonNull(config.get(GUARDIUM_EVENT_KAFKA_TOPIC), "Guardium event kafka topic cannot be null");
            requireNonNull(config.get(CES_TRUST_STORE_PATH), "CES truststore path is null");
            requireNonNull(config.get(CES_TRUST_STORE_PASSWORD), "CES truststore password is null");
        }
        if (finOpsEnabled) {
            requireNonNull(config.get(KAFKA_BROKERS), "Kafka servers cannot be null");
            requireNonNull(config.get(FINOPS_EVENT_KAFKA_TOPIC), "FinOps event kafka topic cannot be null");
        }

        return new QueryEventListener(clusterName, loggerContext, sendToWebsockeCollector, webSocketCollectUrl, trackEventCreated, trackEventCompleted, trackEventCompletedSplit, useMysqlServiceCollector, useAuditCollector, auditUrl, filterConfig, config, tracingEnabled, guardiumEnabled, finOpsEnabled);
    }
